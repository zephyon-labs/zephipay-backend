...
const DEVNET_FINALIZED_RECONCILIATION_GUARD_REQUIRED_TOKENS=["ADD COLUMN transaction_signature","validate_devnet_finalized_reconciliation_truth","devnet_finalized_reconciliation_truth_guard","immutable commitment and provider policy","NEW.observed_at < commitment.committed_at","confirmation_status IS DISTINCT FROM 'finalized'","NEW.slot IS NULL","PROVIDER_REPORTED_FAILURE","JOIN devnet_submission_commitments","FAILED Devnet lifecycle requires durable terminal evidence"];
...
async function main(): Promise<void> {
  ...
  const devnetFinalizedGuardMigration=files.find(file=>file==="022_devnet_finalized_reconciliation_guard.sql");if(!devnetFinalizedGuardMigration)throw new Error("Devnet finalized reconciliation guard migration is missing.");const devnetFinalizedGuardSql=await readFile(path.join(directory,devnetFinalizedGuardMigration),"utf8");for(const token of DEVNET_FINALIZED_RECONCILIATION_GUARD_REQUIRED_TOKENS)if(!devnetFinalizedGuardSql.includes(token))throw new Error(`Devnet finalized reconciliation guard migration is missing required token: ${token}`);for(const forbidden of["terminal_proof_kind","BLOCK_HEIGHT_EXPIRED_HISTORY_ABSENT","TRANSACTION_EXPIRED_NOT_FOUND","finalized_block_height","finalized_context_slot","absence_context_slot"])if(devnetFinalizedGuardSql.includes(forbidden))throw new Error(`Devnet finalized reconciliation guard retains deferred expiry proof schema: ${forbidden}`);
  const e2eMigration=files.find(file=>file==="019_synthetic_e2e_reliability.sql");if(!e2eMigration)throw new Error("Synthetic E2E reliability migration is missing.");const e2eSql=await readFile(path.join(directory,e2eMigration),"utf8");for(const token of E2E_RELIABILITY_REQUIRED_TOKENS)if(!e2eSql.includes(token))throw new Error(`Synthetic E2E migration is missing required token: ${token}`);
