...
export class DevnetOrchestrationService {
  ...
  async submitPrepared(input:Readonly<{executionId:string;actorSubject:string;commitmentId:string;providerIdempotencyKey:string}>){
    ...
    catch(error){const rejected=error instanceof RailProviderOperationError&&error.message==="Devnet RPC provider.";return Object.freeze({attempted:true as const,...await this.repository.recordSubmissionObservation({observationId:this.idFactory(),executionId:input.executionId,actorSubject:input.actorSubject,preparationId:preparation.preparationId,commitmentId:committed.commitment.commitmentId,providerId:this.submissionProvider.identity.providerId,signature:committed.commitment.signature,outcome:rejected?"REJECTED":"UNKNOWN",contactCertainty:"MAY_HAVE_OCCURRED",observedAt:this.clock(),providerErrorCode:rejected?"CONCLUSIVE_PROVIDER_REJECTION":"SUBMISSION_RESPONSE_UNAVAILABLE"})});}
  }
