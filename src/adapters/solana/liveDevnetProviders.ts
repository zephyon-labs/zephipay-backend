...
import{isSupportedSolanaTransactionError}from"./solanaTransactionError";
...
export class IndependentDevnetReconciliationRpc implements SolanaReconciliationRpc,DevnetReconciliationProvider{
  ...
  async observeSignature(signature:string):Promise<SolanaChainObservation&Readonly<{signature:string;historySearched:true;providerId:string;contextSlot:string;confirmationStatus?:string}>>{const result=await this.rpc.call("getSignatureStatuses",[[signature],{searchTransactionHistory:true}]);if(!record(result)||!record(result.context)||!Array.isArray(result.value)||result.value.length!==1)throw new DevnetRpcError("INVALID_RESPONSE",true);const contextSlot=integer(result.context.slot),value=result.value[0],proof={signature,historySearched:true as const,providerId:this.identity.providerId,contextSlot};if(value===null)return Object.freeze({status:"missing",observedAt:this.clock(),...proof});if(!record(value)||!("err"in value)||value.err!==null&&!isSupportedSolanaTransactionError(value.err))throw new DevnetRpcError("INVALID_RESPONSE",true);const slot=integer(value.slot),confirmation=value.confirmationStatus;if(BigInt(slot)>BigInt(contextSlot)||confirmation!=="processed"&&confirmation!=="confirmed"&&confirmation!=="finalized")throw new DevnetRpcError("INVALID_RESPONSE",true);const observedAt=this.clock();if(confirmation!=="finalized")return Object.freeze({status:"pending",observedAt,slot,confirmationStatus:confirmation,...proof});if(value.err!==null)return Object.freeze({status:"failed",observedAt,failedAt:observedAt,slot,confirmationStatus:confirmation,errorCode:"PROVIDER_REPORTED_FAILURE",...proof});return Object.freeze({status:"settled",observedAt,settledAt:observedAt,slot,confirmationStatus:confirmation,...proof});}
}
