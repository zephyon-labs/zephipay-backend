...
function mapReceipt(row: any): ExecutionReceipt { return Object.freeze({receiptId:String(row.receipt_id),paymentIntentId:String(row.payment_intent_id),executionId:String(row.execution_id),actorSubject:String(row.actor_subject),runtimeTransactionId:String(row.runtime_transaction_id),rail:"mock",asset:"USDC",amountUnits:String(row.amount_units),amountDecimals:Number(row.amount_decimals),senderId:String(row.sender_id),recipientId:String(row.recipient_id),recipientSnapshot:row.recipient_snapshot??undefined,memo:row.memo===null?null:String(row.memo),providerReference:row.provider_reference??undefined,settledAt:iso(row.settled_at)!,evidenceType:String(row.evidence_type),evidenceVersion:Number(row.evidence_version),evidence:row.evidence,schemaVersion:1,requestHash:Buffer.from(row.request_hash).toString("hex"),createdAt:iso(row.created_at)!}); }
