...
export class PostgresExecutionRepository implements ExecutionRepository {
  ...
  async claim(statuses: readonly ExecutionStatus[], workerId: string, now: string, leaseExpiresAt: string) { return runWithReliabilityContext({dbOperation:"EXECUTION_CLAIM"},()=>this.tx(async (client) => {
    const result = await client.query(`SELECT * FROM payment_executions WHERE execution_mode='mock_beta' AND status=ANY($1::payment_execution_status[])
...
function mapExecution(row: any): PaymentExecution { return Object.freeze({executionId:String(row.execution_id),paymentIntentId:String(row.payment_intent_id),actorSubject:String(row.actor_subject),status:row.status,version:BigInt(row.version),selectedRail:row.selected_rail,runtimeContractVersion:1,adapterVersion:1,providerIdempotencyKey:String(row.provider_idempotency_key),providerReference:row.provider_reference??undefined,reconciliationReference:row.reconciliation_reference??undefined,attemptCount:Number(row.attempt_count),observationSequence:Number(row.observation_sequence),nextAttemptAt:iso(row.next_attempt_at),lastReconciledAt:iso(row.last_reconciled_at),failureCode:row.failure_code??undefined,failureCategory:row.failure_category??undefined,failureRetryable:row.failure_retryable??undefined,reviewReason:row.review_reason??undefined,settlementEvidence:row.settlement_evidence??undefined,createdAt:iso(row.created_at)!,startedAt:iso(row.started_at),submittedAt:iso(row.submitted_at),settledAt:iso(row.settled_at),failedAt:iso(row.failed_at),cancelledAt:iso(row.cancelled_at),updatedAt:iso(row.updated_at)!,leaseOwner:row.lease_owner??undefined,leaseExpiresAt:iso(row.lease_expires_at)}); }
function mapReceipt(row: any): ExecutionReceipt { return Object.freeze({receiptId:String(row.receipt_id),paymentIntentId:String(row.payment_intent_id),executionId:String(row.execution_id),actorSubject:String(row.actor_subject),runtimeTransactionId:String(row.runtime_transaction_id),rail:row.rail,asset:"USDC",amountUnits:String(row.amount_units),amountDecimals:Number(row.amount_decimals),senderId:String(row.sender_id),recipientId:String(row.recipient_id),recipientSnapshot:row.recipient_snapshot??undefined,memo:row.memo===null?null:String(row.memo),providerReference:row.provider_reference??undefined,settledAt:iso(row.settled_at)!,evidenceType:String(row.evidence_type),evidenceVersion:Number(row.evidence_version),evidence:row.evidence,schemaVersion:1,requestHash:Buffer.from(row.request_hash).toString("hex"),createdAt:iso(row.created_at)!}); }
